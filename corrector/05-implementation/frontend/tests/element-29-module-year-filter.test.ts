...
describe('Element #29 — corrector-modules-form: filtro por año', () => {
  it('filters rows by the year of their legislation after the debounce', async () => {
    const el = mount(makeModuleService(), makeLegislationService(), makeCycleService());
    await flush();

    const filter = el.shadowRoot!.querySelector('[data-element-id="29"]') as HTMLInputElement;
    filter.value = '2020';
