...
function makeTeacherService(overrides: Partial<TeacherService> = {}): TeacherService {
  return {
    ...
    create: async (data) => ({ ok: true, item: { id: 1, username: data.username, role: 'profesor', passwordStatus: 'default', accountLocked: false, failedLoginAttempts: 0, modules: [{ id: 1, name: 'DEW' }] } }),
...
async function fillValidForm(el: CorrectorTeachersForm): Promise<void> {
...
describe('Element #41 — corrector-teachers-form: Guardar button', () => {
  it('clears the form and adds the new row to the table on success', async () => {
    ...
    const usernameAfter = el.shadowRoot!.querySelector('[data-element-id="35"]') as HTMLInputElement;
  ...
  it('shows a loading state while the request is in flight', async () => {
    ...
    const el = mount(
      makeTeacherService({
        create: (data) => new Promise((resolve) => {
          resolveCreate = () => resolve({ ok: true, item: { id: 1, username: data.username, role: 'profesor', passwordStatus: 'default', accountLocked: false, failedLoginAttempts: 0, modules: [] } });
        }),
      }),
    ...
    expect((el.shadowRoot!.querySelector('[data-element-id="41"]') as HTMLButtonElement).disabled).toBe(true);
