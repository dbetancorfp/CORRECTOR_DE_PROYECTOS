...
const MODULES = [
  ...
  { id: 2, name: 'DWEC', weeklyHours: 5, cycleId: 1, cycleName: 'DAW', legislationId: 2, legislationName: 'LOMLOE' },
];
...
describe('Element #30 — corrector-modules-form: filtro por legislación', () => {
  it('filters rows by legislation abbreviation substring, case-insensitively', async () => {
    const el = mount(makeModuleService(), makeLegislationService(), makeCycleService());
    await flush();

    const filter = el.shadowRoot!.querySelector('[data-element-id="30"]') as HTMLInputElement;
    filter.value = 'lomloe';
    ...
    expect(table.textContent).toContain('DWEC');
