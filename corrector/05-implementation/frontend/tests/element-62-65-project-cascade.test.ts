...
describe('Elements #62–#65 — corrector-projects-form: cascade selects', () => {
  it('#64/#65 start disabled and #64 enables once #62/#63 are filled', async () => {
    const el = mount(makeProjectService(), makeLegislationService(), makeCycleService(), makeModuleService());
    await flush();

    expect((el.shadowRoot!.querySelector('[data-element-id="64"]') as HTMLSelectElement).disabled).toBe(true);
    expect((el.shadowRoot!.querySelector('[data-element-id="65"]') as HTMLSelectElement).disabled).toBe(true);
    ...
    expect((el.shadowRoot!.querySelector('[data-element-id="64"]') as HTMLSelectElement).disabled).toBe(false);
    el.remove();
  });

  it('#65 shows only modules belonging to the selected ciclo', async () => {
    const el = mount(makeProjectService(), makeLegislationService(), makeCycleService(), makeModuleService());
    await flush();
  ...
  it('shows error state on all cascade fields when submitted without selection', async () => {
    ...
    expect((el.shadowRoot!.querySelector('[data-element-id="62"]') as HTMLSelectElement).getAttribute('aria-invalid')).toBe('true');
    expect((el.shadowRoot!.querySelector('[data-element-id="63"]') as HTMLSelectElement).getAttribute('aria-invalid')).toBe('true');
    expect((el.shadowRoot!.querySelector('[data-element-id="64"]') as HTMLSelectElement).getAttribute('aria-invalid')).toBe('true');
    expect((el.shadowRoot!.querySelector('[data-element-id="65"]') as HTMLSelectElement).getAttribute('aria-invalid')).toBe('true');
    el.remove();
  });
});
