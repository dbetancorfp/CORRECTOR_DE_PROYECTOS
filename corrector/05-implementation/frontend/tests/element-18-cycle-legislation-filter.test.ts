...
describe('Element #18 — corrector-cycles-form: filtro por legislación (JOIN vía módulos)', () => {
  it('resolves the typed abbreviation substring to matching legislations, case-insensitively', async () => {
    const el = mount(
      makeCycleService({
        list: async (filters) => {
    ...
    const filter = el.shadowRoot!.querySelector('[data-element-id="18"]') as HTMLInputElement;
    filter.value = 'lomloe';
