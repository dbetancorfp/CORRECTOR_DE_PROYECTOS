...
describe('Element #35 — corrector-teachers-form: nombre de usuario', () => {
  ...
  it('accepts a valid username and saves the teacher on submit', async () => {
    let created: unknown = null;
    const el = mount(
      makeTeacherService({ create: async (data) => { created = data; return { ok: true, item: { id: 5, username: data.username, role: 'profesor', passwordStatus: 'default', accountLocked: false, failedLoginAttempts: 0, modules: [] } }; } }),
    ...
    expect(created).toEqual({ username: 'mariagon', password: '12345678', moduleId: 1 });
    el.remove();
  });

  it('shows an "already exists" error for a duplicate username', async () => {
    const el = mount(
      makeTeacherService({ create: async () => ({ ok: false, status: 409, code: 'DUPLICATE' }) }),
      makeLegislationService(),
      makeCycleService(),
      makeModuleService(),
    );
