...
const MODULES = [
  ...
  { id: 2, name: 'RED', weeklyHours: 5, cycleId: 2, cycleName: 'ASIR', legislationId: 2, legislationName: 'LOMLOE' },
];
const TEACHERS = [
...
describe('Element #42 — corrector-teachers-form: filtro por año', () => {
  it('filters rows by the year of their assigned module\'s legislation after the debounce', async () => {
    const el = mount(makeTeacherService(), makeLegislationService(), makeCycleService(), makeModuleService());
    await flush();

    const filter = el.shadowRoot!.querySelector('[data-element-id="42"]') as HTMLInputElement;
    filter.value = '2020';
